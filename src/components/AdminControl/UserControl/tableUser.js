...
class EditableTable extends Component {
  ...
  handleDelete = key => {
    ...
    const url = "https://in.huayuaobo.com:16400/api/user/" + item;
    const opts = {
      method: "DELETE",
  ...
  save(form, key) {
    form.validateFields((error, row) => {
      ...
      if (
        saveItem.password !== null &&
        saveItem.password !== "" &&
        !passwordReg.test(saveItem.password)
      ) {
      ...
      const token = window.sessionStorage.getItem("token");
      const url = "https://in.huayuaobo.com:16400/api/user/" + newItem.username;
      const opts = {
        method: "PUT",
